...
namespace dotnet_registration_api.tests
{
    public class ApiTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        public async Task UpdateUser_ReturnsOk()
        {
            ...
            // Create a unique username to avoid conflicts with other tests
            string uniqueUsername = $"updatetest-{Guid.NewGuid()}";
            
            // First update the username to make it unique
            var initialUpdate = new UpdateRequest()
            {
            ...
            var initialResponse = await _client.PutAsync($"/api/users/{u.Id}", 
                new StringContent(JsonConvert.SerializeObject(initialUpdate), Encoding.UTF8, "application/json"));
            initialResponse.EnsureSuccessStatusCode();
            
            // Now perform the actual test update
            ...
            var response = await _client.PutAsync($"/api/users/{u.Id}", 
            ...
            Assert.Equal(uniqueUsername, updatedUser.Username);
        }
        [Fact]
